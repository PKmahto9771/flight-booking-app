...
const Table = () => {
  ...
  const handleDelete = (e) => {
    dispatch(cancelBooking(parseInt(e.currentTarget.id)));
  };
  console.log(data)
            ...
            <tbody>
              {data.map((data, i) => (
                <tr key={i}>
                  ...
                  <td><button className="btn-xs bg-red-500 text-white" id={`${data.id}`} onClick={(e) => handleDelete(e)}> <MdCancel /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
